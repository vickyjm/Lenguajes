...
% Predicado para chequear que cada elemento del tablero sea un entero entre 1 y 9.
valA([H|[]]) :- integer(H),(H < 10),(H > 0).
valA([H|T]) :- integer(H),(H < 10),(H > 0),valA(T).

% Predicado para chequear si todas las filas del tablero tienen igual longitud.
lenVal([_|[]]).
lenVal([H1|[H2|T]]) :- length(H2,X),(length(H1,X)),lenVal([H2|T]).

% Predicado que verifica si un tablero es valido.
tableroValido([H|[]]) :- valA(H).
tableroValido([H|T]) :- valA(H),tableroValido(T),lenVal([H|T]).

% Predicado que verifica si el numero ubicado en la posicion [X][Y] del tablero coincide con Elem.
numIguales([],_,_,_).
numIguales([_|T],X,Y,Elem) :- X1 is X-1, X > 0, numIguales(T,X1,Y,Elem).
numIguales([H|_],0,Y,Elem) :- Y1 is Y+1, nth(Y1,H,Elem).

% Predicado para calcular el numero de columnas del tablero.
numColums([H|_],Tam) :- length(H,Tam).

% Predicado satoEnPosicion.
